// ============================================================
//  Επίθετα — Θεωρία & Μελέτη (launcher)
//  games/lat-epitheta-kata/theory.js
//
//  Hosts the polished standalone build (theory-study.html) inside the
//  existing #lekt-overlay via an iframe. The standalone page consumes the
//  verified shared dataset (data.js → LATEK_DB) and inherits the active
//  SymposiON theme: we copy the host's --sym-* tokens into the iframe on load.
//
//  Entry points (global): openLatEpithetaKataTheory(), closeLatEpithetaKataTheory()
...
(function () {
  ...
  function resolveSrc() {
    ...
    return (base ? base.replace(/\/?$/, '/') : '') + 'games/lat-epitheta-kata/theory-study.html';
  }
  ...
  window.openLatEpithetaKataTheory = function () {
    const ov = document.getElementById('lekt-overlay');
    if (!ov) return;
    ov.classList.add('active');
    document.body.style.overflow = 'hidden';

    const wrap = document.getElementById('lekt-wrap');
    if (!wrap) return;
    let frame = document.getElementById('lekt-frame');
    if (!frame) {
      ...
      frame.id = 'lekt-frame';
      frame.title = 'Επίθετα — Θεωρία & Μελέτη';
  ...
  window.closeLatEpithetaKataTheory = function () {
    const ov = document.getElementById('lekt-overlay');
