...
/* ── Routing ──────────────────────────────────────────────────
   Hierarchical redirect: (1) switch the underlying admin tab,
   (2) run the panel initializer that builds the sub-module DOM,
   (3) reveal AND focus the specific module/form — not just an anchor. */
function atlasNavigate(feature) {
  if (!feature.tab) { _atlasToast(feature); return; }
  closeAtlas();
  // 1 ▸ switch the underlying admin tab first
  var tabBtn = _ag('admin-tab-btn-' + feature.tab);
  if (typeof adminSwitchTab === 'function') adminSwitchTab(feature.tab, tabBtn);
  ...
  requestAnimationFrame(function() {
    setTimeout(function() { _atlasFocusModule(feature); }, 120);
  });
}
...
function _atlasGoToCurriculumDataset(dsId) {
  closeAtlas();
  var tabBtn = _ag('admin-tab-btn-curriculum');
  if (typeof adminSwitchTab === 'function') adminSwitchTab('curriculum', tabBtn);
