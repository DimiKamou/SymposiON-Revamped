// ============================================================
//  Ανώμαλα Ρήματα Λατινικών — Θεωρία & Μελέτη (launcher)
//  games/lat-anwmala/theory.js
//
//  Hosts the standalone build (theory-study.html) inside the existing
//  #latanwt-overlay via an iframe. The page consumes the verified shared
//  dataset (data.js → LIV_DB) and inherits the active SymposiON theme:
//  we copy the host's --sym-* tokens into the iframe on load.
//
//  Entry points (global): openLatAnwmalaTheory(), closeLatAnwmalaTheory()
...
(function () {
  ...
  function resolveSrc() {
    ...
    return (base ? base.replace(/\/?$/, '/') : '') + 'games/lat-anwmala/theory-study.html';
  }
  ...
  window.openLatAnwmalaTheory = function () {
    const ov = document.getElementById('latanwt-overlay');
    if (!ov) return;
    ov.classList.add('active');
    document.body.style.overflow = 'hidden';

    const wrap = document.getElementById('latanwt-wrap');
    if (!wrap) return;
    let frame = document.getElementById('latanwt-frame');
    if (!frame) {
      wrap.style.position = 'relative';
      wrap.style.padding = '0';
      wrap.innerHTML = '';
      frame = document.createElement('iframe');
      frame.id = 'latanwt-frame';
      frame.title = 'Ανώμαλα Ρήματα — Θεωρία & Μελέτη';
  ...
  window.closeLatAnwmalaTheory = function () {
    const ov = document.getElementById('latanwt-overlay');
