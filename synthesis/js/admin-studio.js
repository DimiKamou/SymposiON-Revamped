...
function gameContentView() {
  ...
  // ΑΔΙΔΑΚΤΟ «Ανθολόγιο» keeps its corpus in its OWN editor
  // (games/adidakto/admin.html — localStorage sym_corpus_v1 + export-to-code).
  if (gm.type === 'adidakto') {
    const url = (typeof window !== 'undefined' && window.APP_BASE ? window.APP_BASE : '') + 'games/adidakto/admin.html';
    return `${crumb()}
      <div class="st-gamehd"><span class="st-row-ic big">${esc(gm.ic || '🏺')}</span>
        <div style="flex:1;min-width:0"><div class="st-gamehd-nm">${esc(gm.label)}</div>
          <div class="st-gamehd-sub">Ανθολόγιο Αδιδάκτων — αρχαία κείμενα από όπου αντλούν τα διαγωνίσματα</div></div>
        <button class="st-open" onclick="window.open('${url}','_blank','noopener')" title="Άνοιγμα σε νέα καρτέλα">↗ Νέα καρτέλα</button></div>
      <div class="cc-note">Πρόσθεσε/επεξεργάσου αδίδακτα κείμενα (συγγραφέας · έργο · δυσκολία · κείμενο). Αλλαγές αποθηκεύονται τοπικά· «Εξαγωγή» δίνει το <code>data/corpus.js</code> για μόνιμη ενσωμάτωση. Το ίδιο ανθολόγιο τροφοδοτεί το Αδίδακτο και την Προσομοίωση Πανελληνίων.</div>
      <iframe class="st-embed" src="${esc(url)}" title="Ανθολόγιο Αδιδάκτων — Επεξεργαστής" loading="lazy"></iframe>`;
  }
