...
function Battle({ node, ctx }) {
  ...
  function win() {
    ...
    if (ctx.hasBoost && ctx.hasBoost("scavenger")) bonus += 6;   // Σκυλεύων boost
  ...
  return (
    <div className="modal__pad modal__pad--battle" style={{ "--node-accent": accent }}>
      ...
      {format !== "volley" && phase === "fight" && !(ctx.hasTrial && ctx.hasTrial("fasting")) ? (
...
function Mystery({ node, ctx }) {
  ...
  function applyEffect(e) {
    ...
    if (e.gamble) {
      ...
      if (e.fortify) { ctx.fortify(e.fortify); msg.push(`+${e.fortify} ἀνώτατο μένος`); }
  ...
  return (
    <div className="modal__pad" style={{ "--node-accent": accent }}>
      <div className="modal__crest">{ev.luck ? "🎲" : node.type === "riddle" ? "𓁹" : "❖"}</div>
      <div className="modal__eyebrow">{ev.luck ? "Τύχη · Fortune" : node.type === "riddle" ? "Αἴνιγμα · Riddle" : "Οἰωνός · Omen"}</div>
      ...
      {resolved ? (
        <>
          {resolved.luckIcon ? <div className="luck__result" style={{ color: resolved.good ? "var(--sym-gold-lt)" : "var(--sym-terra)" }}>{resolved.luckIcon}</div> : null}
          {resolved.luckLabel ? <div className="modal__eyebrow" style={{ color: resolved.good ? "var(--sym-gold-lt)" : "var(--sym-terra)" }}>{resolved.good ? "Εὐλογία" : "Κατάρα"} · {resolved.luckLabel}</div> : null}
        ...
        <div className="luck">
          <div className="luck__wheel" aria-hidden="true">🎲</div>
          {ctx.hasBoost && ctx.hasBoost("fortune") ? <p className="luck__odds">🍀 Τύχη Ἀγαθή — 72% γιὰ εὐλογία</p> : <p className="luck__odds">50 / 50 — εὐλογία ἢ κατάρα</p>}
          <div className="btnrow"><button className="btn btn--gold" onClick={temptFate}>Δοκίμασε τὴν τύχη →</button></div>
        </div>
      ) : ev.riddle ? (
        <>
...
function Shop({ node, ctx }) {
  const stock = useMemo(() => {
    ...
    let mult = (ctx.mods && ctx.mods.shopMult) || 1;
    if (ctx.hasBoost && ctx.hasBoost("haggler")) mult *= 0.75;   // Παζαρευτής boost
