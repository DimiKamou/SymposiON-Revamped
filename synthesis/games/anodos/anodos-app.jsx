...
function freshRun(seed, floors, opts) {
  ...
  return {
    ...
    levels: opts.levels || (window.getSelection ? window.getSelection() : []),
...
function App() {
  ...
  const [subjectGate, setSubjectGate] = useState(null); // syllabus pick (runs before the patron gate)
  const [confirmNew, setConfirmNew] = useState(null);   // discard-in-progress confirmation
  ...
  useEffect(() => { document.documentElement.setAttribute("data-corners", t.corners); }, [t.corners]);
  ...
  // ambient music toggle (synthesised drone)
  useEffect(() => {
    if (t.music === "on") { if (window.startMusic) window.startMusic(); }
    else { if (window.stopMusic) window.stopMusic(); }
  }, [t.music]);
  ...
  // realm preview: when set, force the backdrop's realm so you can see each one without climbing
  const bgAct = (t.realm && t.realm !== "auto") ? Number(t.realm) : currentAct;
  const actDef = ACTS[currentAct - 1] || ACTS[0];
  const bgActDef = ACTS[bgAct - 1] || ACTS[0];

  // act backdrop tint
  useEffect(() => {
    document.documentElement.style.setProperty("--act-tint", bgActDef.tint);
    document.documentElement.setAttribute("data-act", String(bgAct));
  }, [bgAct, bgActDef.tint]);

  // a climb only counts as "in progress" once the player has actually stepped onto the slope
  function newRun(opts) {
    const inProgress = run && run.status === "climbing" && (run.visited || []).length > 0;
    if (inProgress) { setConfirmNew(opts || {}); return; }
    setSubjectGate(opts || {});   // syllabus → patron → climb
  }
  function openSubjects(opts) { setConfirmNew(null); setSubjectGate(opts || {}); }
  ...
  function startRun(extra) {
    ...
    const r = freshRun(seed, floors, { patron: o.patron || "none", anabasis, trials, daily: o.daily, dailyDate: o.dailyDate, levels: o.levels });
    ...
    // ── optional trials (hardship modes chosen at the gate) ──
    if (trials.includes("pauper")) r.drachmes = 0;
    if (trials.includes("glass")) { r.menos.max = Math.max(20, Math.round(r.menos.max / 2)); r.menos.cur = r.menos.max; }
    if (trials.includes("cursed3")) {
      const pool = [...CURSE_POOL];
      for (let i = pool.length - 1; i > 0; i--) { const j = (Math.random() * (i + 1)) | 0; [pool[i], pool[j]] = [pool[j], pool[i]]; }
      const extra = trials.includes("ironwill") ? 2 : 0;
      r.curses = pool.slice(0, 3).map((id) => ({ id, turns: (CURSES[id].turns || 2) + 1 + (mods.curseTurns || 0) + extra }));
    }
  ...
  const ctx = useMemo(() => {
    ...
    return {
      ...
      hasTrial: (id) => !!(run && run.trials && run.trials.includes(id)),
      ...
      useLifeline: (id) => { metaApi.track("lifeline"); metaApi.spendLifeline(id); },
      hasBoost: (id) => !!(run && run.boosts && run.boosts.includes(id)),
      gainDrachmes: (n) => setRun((r) => ({ ...r, drachmes: Math.max(0, r.drachmes + n) })),
      spendDrachmes: (n) => { if (n > 0) metaApi.track("spend", { amt: n }); setRun((r) => ({ ...r, drachmes: Math.max(0, r.drachmes - n) })); },
      ...
      damage: (n) => setRun((r) => {
        ...
        if (cur <= 0) {
          ...
          if (r.boosts && r.boosts.includes("phoenix") && !r.phoenixUsed) return { ...r, menos: { ...r.menos, cur: 25 }, phoenixUsed: true };
      ...
      inflictCurse: (id) => {
        ...
        const ironExtra = (run && run.trials && run.trials.includes("ironwill")) ? 2 : 0;
        const turns = def.turns + (mods.curseTurns || 0) + ironExtra;   // ascension + ironwill trial lengthen miasmata
        setRun((r) => {
  ...
  return (
    <div className={"stage stage--act" + currentAct + (run && run.curses && run.curses.length ? " stage--cursed" : "")}>
      ...
      <Backdrop backdrop={meta.backdrop} particles={meta.particles} act={bgAct} tone={t.tone} />
      ...
      <div className="mapwrap" ref={mapRef}>
        {map && layout ? (
          <div className="map" style={{ height: layout.__height }}>
            ...
            {/* nodes */}
            {map.nodes.map((n) => (
              <NodeDot key={n.id} node={n} pos={layout[n.id]} fr={fr} lang={t.language} reveal={run && (run.patron === "hermes" || (run.boosts && run.boosts.includes("torch")))}
      ...
      {/* syllabus pick — choose which units feed this climb (before the patron gate) */}
      {subjectGate ? <SubjectSelect pending={subjectGate} lang={t.language}
        onConfirm={(ids) => { const o = subjectGate; setSubjectGate(null); setPendingRun({ ...(o || {}), levels: ids }); }}
        onCancel={() => setSubjectGate(null)} /> : null}

      {/* discard-in-progress confirmation */}
      {confirmNew ? <ConfirmNew lang={t.language} onYes={() => openSubjects(confirmNew)} onNo={() => setConfirmNew(null)} /> : null}

      {/* patron-select gate */}
      {pendingRun ? <PatronSelect pending={pendingRun} meta={meta} lang={t.language} onConfirm={(p, trials) => startRun({ patron: p, trials })} onCancel={() => { const o = pendingRun; setPendingRun(null); setSubjectGate({ ...(o || {}) }); }} /> : null}
      ...
      <TweaksPanel>
        ...
        <TweakSelect label="Realm backdrop" value={t.realm}
          options={[{ value: "auto", label: "Auto · follows the climb" }, { value: "1", label: "Α΄ Πεδίον — the pyre" }, { value: "2", label: "Β΄ Τείχη — the walls" }, { value: "3", label: "Γ΄ Ἀκρόπολις — the god-light" }]}
          onChange={(v) => setTweak("realm", v)} />
        ...
        <TweakRadio label="Μουσική · Music" value={t.music} options={["off", "on"]} onChange={(v) => setTweak("music", v)} />
  ...
  return (
    <header className="hud">
      ...
      <div className="hud__run">
        ...
        {run && (run.patron !== "none" || (run.anabasis || 0) > 0 || run.daily || (run.trials && run.trials.length)) ? (
          <div className="hud__runbadges">
            ...
            {(run.trials || []).map((id) => TRIAL_BY_ID[id] ? <span key={id} className="runbadge runbadge--trial">{TRIAL_BY_ID[id].icon} {TRIAL_BY_ID[id].name}</span> : null)}
...
function PatronSelect({ pending, meta, lang, onConfirm, onCancel }) {
  ...
  const toggleTrial = (id) => { if (window.playSfx) window.playSfx("click"); setTrials((ts) => ts.includes(id) ? ts.filter((x) => x !== id) : [...ts, id]); };
  const mult = rewardMultiplier({ patron: sel, trials });
      ...
      <div className="modal" style={{ maxWidth: 620 }}>
        <div className="modal__pad">
          ...
          <div className="trials">
            <div className="trials__head">
              <div className="trials__title"><b>Δοκιμασίαι</b><span>Trials — optional hardship for greater glory</span></div>
              <div className={"trials__mult" + (mult > 1.001 ? " trials__mult--up" : mult < 0.999 ? " trials__mult--down" : "")}>
                <KleosIcon size={16} /> ×{mult.toFixed(2)} κλέος
              </div>
            </div>
            <div className="trials__grid">
              {TRIALS.map((tr) => {
                const on = trials.includes(tr.id);
                return (
                  <button key={tr.id} className={"trialchip" + (on ? " trialchip--on" : "")} onClick={() => toggleTrial(tr.id)}
                    title={lang === "en" ? tr.enDesc : tr.desc}>
                    <span className="trialchip__ic">{tr.icon}</span>
                    <span className="trialchip__tx"><b>{tr.name}</b><span>{lang === "en" ? tr.enDesc : tr.desc}</span></span>
                    <span className="trialchip__mult">+{Math.round(tr.mult * 100)}%</span>
                  </button>
                );
              })}
            </div>
            <div className="trials__note">{sel === "none"
              ? "Ἀστεγος — χωρὶς θεὸ κερδίζεις περισσότερο κλέος. Ἡ βοήθεια θεοῦ τὸ μειώνει."
              : "Ἡ χάρη τοῦ θεοῦ μειώνει τὸ κλέος ×0,82 — οἱ Δοκιμασίες τὸ ἀνεβάζουν ξανά."}</div>
          </div>
          <div className="btnrow">
            <button className="btn btn--gold" onClick={() => onConfirm(sel, trials)}>Ξεκίνα τὴν ἄνοδο →</button>
...
// ── Discard-in-progress confirmation ─────────────────────────────────
function ConfirmNew({ lang, onYes, onNo }) {
  const en = lang === "en";
  return (
    <div className="scrim scrim--top" onClick={(e) => { if (e.target.classList.contains("scrim")) onNo(); }}>
      <div className="modal" style={{ maxWidth: 440 }}>
        <div className="modal__pad">
          <div className="modal__crest" style={{ borderColor: "var(--sym-terra)", boxShadow: "0 0 24px color-mix(in srgb, var(--sym-terra) 40%, transparent)" }}>⚠</div>
          <div className="modal__eyebrow" style={{ color: "var(--sym-terra)" }}>{en ? "Abandon the climb?" : "Ἐγκατάλειψη ἀνόδου;"}</div>
          <h2 className="modal__title">{en ? "Start a new ascent?" : "Νέα ἄνοδος;"}</h2>
          <p className="modal__sub">{en ? "Your current climb will be lost — this cannot be undone." : "Ἡ τρέχουσα ἄνοδος θὰ χαθεῖ — δὲν ἀναιρεῖται."}</p>
          <div className="btnrow">
            <button className="btn btn--ghost" onClick={onNo}>{en ? "Keep climbing" : "Συνέχισε τὴν ἄνοδο"}</button>
            <button className="btn btn--gold" onClick={onYes}>{en ? "New ascent →" : "Νέα ἄνοδος →"}</button>
          </div>
        </div>
      </div>
    </div>
