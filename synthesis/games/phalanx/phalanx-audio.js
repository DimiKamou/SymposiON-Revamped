...
(function () {
  class PhalanxAudioEngine {
    ...
    wrong() {
      if (this.muted || !this._init()) return;
      const ctx = this._ctx, t = ctx.currentTime;
      // the shield CRACKS — a sharp splintering snap before the doom-drums
      this._noise(0.05, 0.28, 3400, 3.0, t);
      this._noise(0.09, 0.18, 1900, 2.2, t + 0.015);
      this._tone('square', 620, 210, 0.08, 0.06, t + 0.01);
      // doom-drums: boom … boom — the line breaks
      this._drum(t + 0.06, 130, 42, 0.4, 0.36);
      this._drum(t + 0.23, 110, 36, 0.5, 0.32);
      this._tone('sawtooth', 200, 70, 0.34, 0.16, t + 0.06);
      this._noise(0.22, 0.12, 180, 0.6, t + 0.10);
    }
    standoff() {
      if (this.muted || !this._init()) return;
      const ctx = this._ctx, t = ctx.currentTime;
