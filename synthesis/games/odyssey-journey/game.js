...
(function () {
  ...
  let pathMats = [];            // course-line shader mats (uTime)
  ...
  window.destroyOdysseyJourney = function () {
    ...
    foamMats = []; pathMats = []; cloudSprites = []; shootingStars = []; bursts = [];
    skyMat = null; glitterMesh = null; oceanMesh = null; oceanPositions = null;
  ...
  function firePendingBurst() {
    ...
    if (g) spawnBurst(g.position.x, (g.userData.locH || 4) + 2.5, g.position.z, LAUREL_HEX);
    pendingBurstLocId = null;
  }
  ...
  function buildGame() {
    ...
    window.__ojDbg = { scene, glitterMesh, cloudSprites, skyMat }; // TEMP DEBUG
  ...
  function buildMoon() {
    ...
    const moonLight = new T.DirectionalLight(0xc8ddf8, 0.85);
  ...
  function buildClouds() {
    ...
    for (let i = 0; i < 6; i++) {
      const mat = new T.SpriteMaterial({
        ...
        color: new T.Color(0x22364f),   // dark slate haze, not white puffs
      });
  ...
  function buildOcean() {
    ...
    // per-vertex color: deep abyssal teal in the distance grading to a
    // brighter Aegean turquoise near the centre where the ship sails
    const cnt = oceanPositions.count;
    const cols = new Float32Array(cnt * 3);
    const deep    = new T.Color(0x04141f);
    const shallow = new T.Color(0x0f3d5c);
    for (let i = 0; i < cnt; i++) {
    ...
    // High shininess + restrained specular: the moon reads as tight
    // glints on wave crests, not a broad white sheet across the sea.
    const mat = new T.MeshPhongMaterial({
      vertexColors: true,
      emissive:  new T.Color(0x03101c),
      emissiveIntensity: 0.35,
      shininess: 620,
      specular:  new T.Color(0x14222f),
    });
  ...
  // Shimmering moon-path on the water — a stretched additive plane whose
  // vertices ride the SAME wave field as the CPU ocean, so the glints sit
  // on the swell instead of being clipped into pale sheets by the troughs.
  function buildMoonGlitter() {
    const T = THREE;
    const mat = new T.ShaderMaterial({
      transparent: true, depthWrite: false, blending: T.AdditiveBlending,
      uniforms: { uTime: { value: 0 }, uColor: { value: new T.Color(0xa8ccf4) } },
      vertexShader: `
        varying vec2 vUv;
        void main(){
          vUv = uv;
          // flat sheet held just above the highest wave crest — riding the
          // CPU swell caused fold/clipping bands where the two grids
          // sampled the same wave field at different resolutions
          gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);
        }
      `,
      fragmentShader: `
        ...
        void main(){
          vec2 p = vUv - 0.5;
          // moonglade: a slim ribbon, narrow at the horizon, fanning gently
          float lane = smoothstep(0.13, 0.0, abs(p.x) * (0.75 + 1.9 * vUv.y));
          float body = smoothstep(0.50, 0.06, length(p * vec2(1.25, 0.75)));
          // fade BOTH ends — far, where perspective packs many additive
          // sparks into few pixels (they'd sum into a solid white mass),
          // and near, so the glade dies out before reaching the islands.
          float fade = smoothstep(0.95, 0.35, vUv.y) * smoothstep(0.02, 0.24, vUv.y);
          // fine broken glints drifting on the swell (cells small enough to
          // read as sparkle, never as mosaic tiles). Sparse threshold so the
          // path is mostly dark water with darting highlights.
          float g  = hash(vec2(floor(vUv.y * 220.0), floor(vUv.x * 130.0)));
          float g2 = hash(vec2(floor(vUv.y * 90.0) + 7.0, floor(vUv.x * 55.0)));
          float spark = step(0.945, fract(g + uTime * (0.35 + g * 0.5)))
                      + step(0.96, fract(g2 - uTime * 0.22)) * 0.6;
          float shimmer = 0.05 + spark * 1.0;
          float a = min(lane * body * fade * shimmer * 0.33, 0.33);
          gl_FragColor = vec4(uColor, a);
        }
      `
    });
    const geo = new T.PlaneGeometry(24, 54, 1, 1);
    geo.rotateX(-Math.PI / 2);
    glitterMesh = new T.Mesh(geo, mat);
    // open water east of the archipelago (islands live in x -16..16,
    // z -18..12), running toward the moon on the horizon; y sits just
    // above the tallest possible wave crest (swell amplitudes sum to 1.0)
    glitterMesh.position.set(29, 1.06, -24);
    glitterMesh.rotation.y = 0.5;
  ...
  function buildLighting() {
    ...
    scene.add(new T.HemisphereLight(0x243b66, 0x141006, 0.5));
    scene.add(new T.AmbientLight(0x101f38, 0.45));
    ...
    const rim = new T.DirectionalLight(0x4a6ea0, 0.28);
  ...
  function buildIslands() {
    ...
    ODYSSEY_LOCATIONS.forEach((loc, idx) => {
      ...
      const pillarMat = new T.ShaderMaterial({
        ...
        `,
        fragmentShader: `
          varying vec2 vUv; uniform vec3 uColor; uniform float uTime; uniform float uPhase;
          void main(){
            float fadeTop = pow(1.0 - vUv.y, 1.7);
            float fadeBot = smoothstep(0.0, 0.12, vUv.y);
            float breathe = 0.78 + 0.22 * sin(uTime * 1.6 + uPhase);
            float a = fadeTop * fadeBot * 0.30 * breathe;
            gl_FragColor = vec4(uColor, a);
          }
        `
      });
  ...
  function buildShip() {
    ...
    const sailMat  = new T.MeshPhongMaterial({
      ...
      emissive: 0x332211, emissiveIntensity: 0.22,
    });
    ...
    sailGeo = new T.PlaneGeometry(1.02, 1.55, 6, 8);
    ...
    lanternGlow.scale.set(1.7, 1.7, 1);
  ...
  function buildWake() {
    ...
    wakeParticles = [];
    for (let i = 0; i < 10; i++) {
      const w = new T.Mesh(new T.PlaneGeometry(0.5, 0.32), wakeMat.clone());
  ...
  function buildPaths() {
    ...
    for (let i = 0; i < order.length - 1; i++) {
      ...
      const mat = new T.ShaderMaterial({
        ...
        uniforms: {
          uTime:  { value: 0 },
          uColor: { value: new T.Color(0x86b8de) },
        },
        vertexShader: `
        ...
        `,
        fragmentShader: `
          varying float vLd; uniform float uTime; uniform vec3 uColor;
          void main(){
  ...
  function spawnBurst(x, y, z, colorHex) {
    ...
    const mat = new T.PointsMaterial({
      ...
      color: colorHex, size: 0.55, sizeAttenuation: true,
      transparent: true, opacity: 1,
      blending: T.AdditiveBlending, depthWrite: false,
    });
    const points = new T.Points(geom, mat);
    scene.add(points);
    bursts.push({ points, vel, life: 0, max: 1.7 });
  }
  ...
  function animate() {
    ...
    // Sky / stars / glitter / paths / foam — shared clocks
    ...
    if (glitterMesh) glitterMesh.material.uniforms.uTime.value = t;
    for (let i = 0; i < pathMats.length; i++) pathMats[i].uniforms.uTime.value = REDUCED ? 0 : t;
    ...
    locationGroups.forEach((g, i) => {
      ...
      if (g.userData.ring2) g.userData.ring2.rotation.z = -t * 0.3 + i;
      if (g.userData.pillarMat) g.userData.pillarMat.uniforms.uTime.value = t;
      // Rising beacon motes
  ...
  function animateCamera(delta, t) {
    ...
    if (!REDUCED) {
      // intro sweep: glide down from a wide establishing shot
      if (camIntroT < 2.6) {
        camIntroT += delta;
  ...
  function animateShip(delta, t) {
    ...
    if (sailGeo && sailBase) {
      ...
      for (let i = 0; i < attr.count; i++) {
        ...
        const edge = 1 - Math.min(Math.abs(bx) / 0.51, 1);          // pinned at yard ends
        const belly = Math.sin((by / 1.55 + 0.5) * Math.PI);         // pinned top & foot
        attr.setZ(i, (0.16 * full + Math.sin(t * 1.6 + by * 2.2) * 0.05) * edge * belly);
      }
      attr.needsUpdate = true;
  ...
  function animateWake(t) {
    ...
    wakeParticles.forEach((w, i) => {
      ...
      w.material.opacity = (1 - age) * (shipMoving ? 0.4 : 0.24);
      const sc = 0.5 + age * 2.4;
      w.scale.set(sc, sc, 1);
    });
  }
