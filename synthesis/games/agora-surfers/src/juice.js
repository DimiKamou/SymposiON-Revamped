...
/* ───────────────────────────────────────────────────────────────
   RINGS — pooled expanding ground rings (landing dust ring, Talos
   stomp shockwave, power-up claim). One shared geometry; tiny fixed
   pool of additive materials so there are zero per-frame allocations.
─────────────────────────────────────────────────────────────── */
const _ringGeo = new THREE.RingGeometry(0.82, 1, 30);
...
export class Rings {
  constructor(scene, opts = {}) {
    ...
    for (let i = 0; i < this.max; i++) {
      ...
      m.renderOrder = 4;
      scene.add(m);
      this.items.push({ mesh: m, life: 0, maxLife: 1, r0: 0.3, r1: 1.6, a0: 0.5 });
    }
    this.head = 0;
  }

  /* spawn an expanding ring at (x,y,z) */
  spawn(x, y, z, o = {}) {
    if (REDUCED_MOTION && !o.force) return;
    const it = this.items[this.head]; this.head = (this.head + 1) % this.max;
    it.life = it.maxLife = o.life || 0.42;
    it.r0 = o.r0 !== undefined ? o.r0 : 0.3;
    it.r1 = o.r1 !== undefined ? o.r1 : 1.7;
    it.a0 = o.alpha !== undefined ? o.alpha : 0.5;
    it.mesh.material.color.set(o.color !== undefined ? o.color : 0xE8D9B0);
  ...
  update(dt, dz = 0) {
    for (const it of this.items) {
      if (it.life <= 0) { if (it.mesh.visible) it.mesh.visible = false; continue; }
      it.life -= dt;
      ...
      it.mesh.material.opacity = it.a0 * (1 - u) * (1 - u);
      it.mesh.position.z += dz;                          // recede with the road
    }
  }
}
