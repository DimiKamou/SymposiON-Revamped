  ...
  class LatinUnitPanel extends React.Component {
    ...
    themes = {
      A: {
        ...
        '--line': '#dae0d3',
        '--line2': '#e7ebe1',
        '--accent': 'oklch(0.47 0.10 168)',
        '--accent2': 'oklch(0.42 0.11 168)',
        '--on-accent': '#f4faf2',
        '--hl': 'oklch(0.9 0.13 116)',
        ...
        '--nav-w': '100%',
        '--nav-wrap': 'wrap',
        '--nav-pad': '11px clamp(16px,3vw,32px)',
        '--nav-bb': '1px',
        '--nav-br': '0px',
        '--maxw': '1060px'
      },
      B: {
        ...
        '--accent2': 'oklch(0.49 0.17 264)',
        '--on-accent': '#ffffff',
        '--hl': 'oklch(0.9 0.08 250)',
        '--font-ui': "'IBM Plex Sans',system-ui,sans-serif",
        '--font-serif': "'IBM Plex Sans',system-ui,sans-serif",
        '--font-latin': "'IBM Plex Serif',Georgia,serif",
        '--font-mono': "'IBM Plex Mono',monospace",
        '--r': '12px',
        '--r-lg': '18px',
        '--body-dir': 'row',
    ...
    darkOverrides = {
      A: {
        ...
        '--accent2': 'oklch(0.68 0.13 165)',
        '--on-accent': '#0f130d',
        '--hl': 'oklch(0.5 0.13 116)'
      },
      B: {
        '--bg': '#0f151b',
        '--panel': '#18202a',
        ...
        '--accent2': 'oklch(0.68 0.15 264)',
        '--on-accent': '#0a0e14',
        '--hl': 'oklch(0.46 0.12 255)'
      }
    };
    roleDefs = [{
      re: /Ρήμα/,
    ...
    componentWillUnmount() {
      ...
      document.removeEventListener('focusout', this.onEditBlur);
      window.removeEventListener('resize', this._onResize);
    }
    componentDidUpdate() {
      this.applyTheme();
      if (this._scrollWanted) {
    ...
    load() {
      const apply = () => {
        try {
          if (window.LATIN_UNIT_ERR) {
            this.setState({
              err: String(window.LATIN_UNIT_ERR),
              loaded: true
            });
            return;
          }
          ...
          if (!orig) {
            this.setState({
              err: 'Δεν βρέθηκαν δεδομένα ενότητας.',
              loaded: true
            });
            return;
          }
          ...
          try {
            const saved = localStorage.getItem('latin-unit-' + U.number);
            if (saved) {
              const sv = JSON.parse(saved);
              if (sv && sv.periods && sv.dataVersion === U.dataVersion) U = sv;
            }
          } catch (e) {}
          this.assignIds(U);
          this.setState({
          ...
          try {
            document.title = 'Λατινικά · Ενότητα ' + U.number + ' — ' + (U.title || '');
          } catch (e) {}
        } catch (err) {
      ...
      if (window.LATIN_UNIT || window.LATIN_UNIT_ERR) apply();else window.addEventListener('latin-unit-ready', apply, {
        once: true
      });
    }
    applyTheme() {
      ...
      const base = this.themes[dir] || this.themes.A;
      const t = this.state.dark ? Object.assign({}, base, this.darkOverrides[dir] || {}) : base;
    ...
    clauseColor(key) {
      if (!key || key === 'kyria') return 'var(--accent)';
      const m = this.clauseMeta[key];
      const hue = m && m.hue != null ? m.hue : 250;
      return 'oklch(' + (this.state.dark ? 0.74 : 0.56) + ' 0.15 ' + hue + ')';
    }
    composeRole(t) {
      return (t.r || '') + (t.to ? ' ' + t.to : '');
    }
    roleColorL() {
      return this.state.dark ? 0.76 : 0.5;
    }
    roleColorsFor(role) {
      if (!role) return ['var(--muted)'];
      const L = this.roleColorL();
      const out = [];
      this.roleDefs.forEach(d => {
        if (d.re.test(role)) out.push('oklch(' + L + ' 0.16 ' + d.hue + ')');
      });
      if (!out.length) out.push('var(--muted)');
      return out;
    }
    splitGrad(cols) {
      if (cols.length < 2) return cols[0];
      const seg = 100 / cols.length;
      return 'linear-gradient(135deg,' + cols.map((c, i) => c + ' ' + (seg * i).toFixed(1) + '% ' + (seg * (i + 1)).toFixed(1) + '%').join(',') + ')';
    }
    swatchStyle(cols) {
      return {
        width: '11px',
        ...
        background: cols.length > 1 ? this.splitGrad(cols) : cols[0]
      };
    }
    toggleDark = () => {
      this.setState(s => ({
    ...
    buildGroups() {
      ...
      if (!u || !u.periods) return G;
      const L = this.state.dark ? 0.72 : 0.5;
      const c = hue => 'oklch(' + L + ' 0.16 ' + hue + ')';
      let pi = 0;
      const nextCol = () => c(this.groupPalette[pi++ % this.groupPalette.length]);
      const isInf = t => !!(t.g && /απαρέμφατο/.test(t.g));
      const isConn = t => !!(t.r && t.r.trim() === 'Σύνδεσμος');
      const norm = x => (x || '').replace(/[^A-Za-z]/g, '').toLowerCase();
      const headName = to => {
        if (!to) return '';
        let x = to.replace(/\([^)]*\)/g, ' ').replace(/^\s*(στον|στη[νς]?|στους|στα|στο)\s+/, '');
        return norm(x.trim().split(/[\s,]+/)[0]);
      };
      const visit = cl => {
        const clauseCol = nextCol();
        G.cl.set(cl, clauseCol);
        const infCols = {};
        cl.kids.forEach(t => {
          if (!t.kids && isInf(t)) infCols[norm(t.l)] = nextCol();
        });
        cl.kids.forEach(t => {
          if (t.kids || t.plain) return;
          if (isConn(t)) {
            G.tok.set(t, ['var(--muted)']);
            return;
          }
          if (isInf(t)) {
            G.tok.set(t, [clauseCol, infCols[norm(t.l)]]);
            return;
          }
          const hn = headName(t.to);
          if (hn && infCols[hn]) {
            G.tok.set(t, [infCols[hn]]);
            return;
          }
        ...
        cl.kids.forEach(t => {
          if (!t.kids && t.l) {
            const fw = norm(t.l.split(/\s+/)[0]);
            if (nameToId[fw] == null) nameToId[fw] = t._id;
          }
        });
        cl.kids.forEach(t => {
          if (t.kids || t.plain) return;
          const hn = headName(t.to);
          if (hn && nameToId[hn] != null && nameToId[hn] !== t._id) G.head.set(t, nameToId[hn]);
        });
        cl.kids.forEach(t => {
          if (t.kids) visit(t);
        });
      };
      u.periods.forEach(p => p.kids.forEach(t => {
        if (t.kids) visit(t);else if (t.r && !t.plain) G.tok.set(t, ['var(--muted)']);
      }));
      return G;
    }
    textColorStyle(cols) {
      ...
      const seg = 100 / cols.length;
      const stops = cols.map((x, i) => x + ' ' + (seg * i).toFixed(1) + '% ' + (seg * (i + 1)).toFixed(1) + '%').join(',');
      return {
    ...
    assignIds(u) {
      ...
      const walk = arr => {
        ...
        arr.forEach(t => {
          t._id = i++;
          if (t.kids) walk(t.kids);
        });
      };
      (u && u.periods || []).forEach(p => walk(p.kids));
    }
    _persist(u) {
      try {
        localStorage.setItem('latin-unit-' + u.number, JSON.stringify(u));
      } catch (e) {}
    }
    editData(mut) {
    ...
    setByPath(o, path, val) {
      ...
      x[last] = last === 'number' ? parseInt(val, 10) || x[last] : val;
    }
    commitPath = (path, val) => {
      this.editData(u => this.setByPath(u, path, val));
    };
    onEditBlur = e => {
      ...
      if (!p) return;
      this.commitPath(p, (el.textContent || '').trim());
    };
    editToken = (field, e) => {
      ...
      const walk = arr => arr.forEach(t => {
        if (t._id === id) t[field] = val;
        if (t.kids) walk(t.kids);
      });
      u.periods.forEach(p => walk(p.kids));
      this._persist(u);
      const pk = {
        r: 'rawR',
        to: 'toRaw',
    ...
    toggleAdmin = () => {
      if (this.state.admin) {
        ...
        try {
          localStorage.removeItem('latinAdmin');
        } catch (e) {}
        return;
      }
    ...
    exportData = () => {
      ...
      const strip = arr => {
        ...
        arr.forEach(t => {
          delete t._id;
          if (t.kids) strip(t.kids);
        });
      };
      (u.periods || []).forEach(p => strip(p.kids));
      const js = '// ' + (u.course || '') + ' — Ενότητα ' + u.number + '\nexport const UNIT = ' + JSON.stringify(u, null, 2) + ';\n\nexport default UNIT;\n';
      const blob = new Blob([js], {
        type: 'text/javascript'
      });
      const a = document.createElement('a');
      a.href = URL.createObjectURL(blob);
      a.download = 'unit' + u.number + '.js';
    ...
    resetData = () => {
      if (!window.confirm('Επαναφορά αρχικών δεδομένων; Θα χαθούν οι αλλαγές σε αυτό το πρόγραμμα περιήγησης.')) return;
      try {
        localStorage.removeItem('latin-unit-' + this.state.unit.number);
      } catch (e) {}
      const U = JSON.parse(JSON.stringify(this._origUnit || this.state.unit));
      this.assignIds(U);
      this.setState({
        unit: U,
        editId: null
    ...
    revealLine = i => {
      if (!this.state.hideGreek) return;
      this.setState(s => ({
    ...
    showPop(t, el) {
      ...
      this.setState({
        editId: t._id != null ? t._id : null,
        pop: {
          show: true,
          x: Math.round(r.left + r.width / 2),
          y: above ? Math.round(r.top - 10) : Math.round(r.bottom + 10),
          tf: above ? 'translate(-50%,-100%)' : 'translate(-50%,0)',
          l: t.l,
          r: this.composeRole(t),
          g: t.g || '',
          d: t.d || '',
          note: t.note || '',
          rawR: t.r || '',
    ...
    hidePop() {
      ...
      if (this.state.pop.show) this.setState(s => ({
        pop: {
          ...s.pop,
          show: false
        }
    ...
    togglePin(t, el) {
      if (!t || !t.r) return;
      const s = this.state;
      if (s.pinned && s.pop.show && s.pop.l === t.l) {
        this.setState(st => ({
          pinned: false,
          pop: {
            ...st.pop,
    ...
    doScroll() {
      const c = this.scrollRef.current;
      if (!c || !this.state.highlight) return;
      const el = c.querySelector('[data-wkey="' + this.state.highlight + '"]');
      if (el) {
        const cr = c.getBoundingClientRect(),
          er = el.getBoundingClientRect();
        const delta = er.top - cr.top - c.clientHeight / 2 + er.height / 2;
    ...
    drawArrow(fromEl) {
      ...
      const headEl = cont.querySelector('[data-tid="' + hid + '"]');
      if (!headEl) {
        path.style.opacity = 0;
        return;
      }
      const cr = cont.getBoundingClientRect(),
        ...
        b = headEl.getBoundingClientRect();
    ...
    drawAllArrows() {
      ...
      if (!cont) return;
      const cr = cont.getBoundingClientRect();
      const els = [].slice.call(cont.querySelectorAll('[data-tid]'));
      const byId = {};
      els.forEach(e => {
    ...
    buildText() {
      const s = this.state,
        u = s.unit;
      if (!u || !u.periods) return null;
      const h = React.createElement,
        F = React.Fragment,
        self = this,
        syn = s.showSyntax;
      const G = self.buildGroups();
      let idx = 0;
      const decor = (t, c0) => {
        const r = t.r || '',
          g = t.g || '';
        if (/^Ρήμα/.test(r)) return {
          border: '1.7px solid ' + c0,
          borderRadius: '50%',
          padding: '1px 8px 2px'
        };
        if (/απαρέμφατο|μετοχ/.test(g)) return {
          textDecorationLine: 'underline',
          textDecorationStyle: 'solid',
          textDecorationColor: c0,
          textDecorationThickness: '2px',
          textUnderlineOffset: '4px'
        };
        if (/Εμπρόθ/.test(r)) return {
          textDecorationLine: 'underline',
      ...
      const word = (t, ck) => {
        ...
        if (syn && cols) {
          Object.assign(latinStyle, self.textColorStyle(cols), decor(t, c0));
        }
        const latin = h('span', {
          style: latinStyle
        }, t.l + (t.a || ''));
        ...
        if (syn && t.r) {
          const chip = h('span', {
            style: {
              ...
              lineHeight: 1.15,
              color: c0,
              background: 'color-mix(in oklab,' + c0 + ' 15%,transparent)',
              border: '1px solid color-mix(in oklab,' + c0 + ' 32%,transparent)',
              borderRadius: '4px',
              padding: '0 4px 1px',
              marginBottom: '5px',
              whiteSpace: 'nowrap'
      ...
      const node = (n, ck) => {
        if (n.kids) {
          ...
          n.kids.forEach((c, i) => {
            if (i > 0) items.push(' ');
            items.push(node(c, n.key || ck));
          });
          if (!syn) return h(F, {
            key: 'c' + idx++
          }, items);
          const col = G.cl.get(n) || self.clauseColor(n.key);
          const main = n.type !== 'sub';
          const brk = ch => h('span', {
            style: {
              color: col,
              fontWeight: 800,
          ...
          }, n.label || (main ? 'Κύρια' : 'Δευτ.'));
          return h('span', {
            key: 'c' + idx++
          }, brk(main ? '[' : '('), label, ...items, ' ', brk(main ? ']' : ')'));
        }
        return word(n, ck);
      };
      const periods = u.periods.map((p, pi) => {
        ...
        p.kids.forEach((c, i) => {
          if (i > 0) inner.push(' ');
          inner.push(node(c, null));
        });
        const num = syn ? h('span', {
          style: {
      ...
      const svg = h('svg', {
        ...
        ref: self.arrowAllRef
      }), h('path', {
        ref: self.arrowPathRef,
        d: '',
        fill: 'none',
        stroke: 'var(--accent)',
        strokeWidth: '2',
    ...
    buildAnalysis() {
      ...
      const visit = cl => {
        n++;
        const clauseCol = G.cl.get(cl) || self.clauseColor(cl.key);
        const items = [];
        const isVerb = c => !c.kids && !!c.r && c.r.indexOf('Ρήμα') === 0;
        cl.kids.forEach(c => {
          if (!c.kids && !c.plain && c.r) {
            const v = isVerb(c);
            const cols = G.tok.get(c) || [clauseCol];
            const lStyle = Object.assign({
              fontFamily: 'var(--font-latin)',
              fontStyle: 'italic',
              fontWeight: 700
            }, self.textColorStyle(cols), v ? {
              textDecorationLine: 'underline',
              textDecorationColor: cols[0],
              textDecorationThickness: '2px',
              textUnderlineOffset: '3px'
            } : {});
            items.push({
              l: c.l,
              role: self.composeRole(c),
              lStyle,
              swatch: self.swatchStyle(cols)
            });
          }
        });
        const sentenceParts = cl.kids.map(c => {
          if (c.kids) return {
            t: '(…)',
            style: {
              color: 'var(--muted)'
          ...
          return {
            t,
            style: Object.assign({}, self.textColorStyle(cols), v ? {
              textDecorationLine: 'underline',
              textDecorationColor: cols[0],
              textDecorationThickness: '2px',
              textUnderlineOffset: '3px',
        ...
        out.push({
          key: 'a' + n,
          n,
          label: cl.label || (cl.type === 'sub' ? 'Δευτ.' : 'Κύρια'),
          note: cl.note || '',
          items,
          sentenceParts,
          card: {
            ...
            border: '1px solid var(--line)',
            borderLeft: '4px solid ' + clauseCol,
            borderRadius: 'var(--r)',
            padding: '13px 16px'
          },
          badge: {
      ...
      u.periods.forEach(p => p.kids.forEach(c => {
        if (c.kids) visit(c);
      }));
      return out;
    }
    renderVals() {
      ...
      const parts = this.partDefs.filter(p => (p.id !== 'transforms' || hasTransforms) && (p.id !== 'etym' || hasEtym)).map(p => {
        ...
        return {
          ...
          rn: p.rn,
          label: p.label,
          onSelect: () => this.setPart(p.id),
          style: {
            display: 'flex',
            alignItems: 'center',
            gap: '10px',
            width: s.dir === 'B' ? '100%' : 'auto',
            textAlign: 'left',
            cursor: 'pointer',
            fontFamily: 'var(--font-ui)',
            fontSize: '14px',
      ...
      const nouns = (ready ? u.nouns : []).map((kl, ki) => {
        const groups = kl.groups.map((g, gi) => ({
          gender: g.gender,
          items: g.items.map((it, ii) => ({
            form: it.form,
            note: it.note || '',
            onJump: jump(it.form),
            pForm: 'nouns.' + ki + '.groups.' + gi + '.items.' + ii + '.form',
            pNote: 'nouns.' + ki + '.groups.' + gi + '.items.' + ii + '.note'
          })).filter(it => inc(it.form, it.note))
        ...
        return {
          ...
          cardStyle: Object.assign({
            flex: Math.max(2, count) + ' 1 148px'
          }, boxBase)
        };
      }).filter(kl => kl.groups.length);
      const adjectives = (ready ? u.adjectives : []).map((kl, ki) => {
        const items = kl.items.map((it, ii) => ({
          form: it.form,
          note: it.note || '',
          onJump: jump(it.form),
          pForm: 'adjectives.' + ki + '.items.' + ii + '.form',
          pNote: 'adjectives.' + ki + '.items.' + ii + '.note'
        })).filter(it => inc(it.form, it.note));
        return {
          kl: kl.kl,
          items,
          cardStyle: Object.assign({
            flex: Math.max(2, items.length) + ' 1 160px'
          }, boxBase)
        };
      }).filter(kl => kl.items.length);
      const comparatives = (ready ? u.comparatives : []).map((kl, ki) => ({
        kl: kl.kl,
        rows: kl.rows.map((r, ri) => ({
          pos: r.pos,
          comp: r.comp,
          sup: r.sup,
          onJump: jump(r.pos),
          pPos: 'comparatives.' + ki + '.rows.' + ri + '.pos',
          pComp: 'comparatives.' + ki + '.rows.' + ri + '.comp',
          pSup: 'comparatives.' + ki + '.rows.' + ri + '.sup'
        })).filter(r => inc(r.pos, r.comp, r.sup))
      })).filter(kl => kl.rows.length);
      const pronouns = (ready ? u.pronouns : []).map((p, pi) => ({
        form: p.form,
        kind: p.kind,
        extra: p.extra || '',
        onJump: jump(p.form),
        pForm: 'pronouns.' + pi + '.form',
        pKind: 'pronouns.' + pi + '.kind',
        pExtra: 'pronouns.' + pi + '.extra'
      })).filter(p => inc(p.form, p.kind, p.extra));
      const verbs = (ready ? u.verbs : []).map((sz, zi) => {
        const rows = sz.rows.map((r, ri) => ({
          pres: r.pres,
          perf: r.perf,
          sup: r.sup,
          inf: r.inf,
          note: r.note || '',
          onJump: jump(r.pres),
          pPres: 'verbs.' + zi + '.rows.' + ri + '.pres',
          pPerf: 'verbs.' + zi + '.rows.' + ri + '.perf',
          pSup: 'verbs.' + zi + '.rows.' + ri + '.sup',
          pInf: 'verbs.' + zi + '.rows.' + ri + '.inf',
          pNote: 'verbs.' + zi + '.rows.' + ri + '.note'
        })).filter(r => inc(r.pres, r.perf, r.sup, r.inf, r.note));
        return {
          syz: sz.syz,
          rows,
      ...
      const align = (ready ? u.alignment : []).map((a, i) => {
        const bg = i % 2 === 1 ? 'var(--panel2)' : 'var(--panel)';
        const masked = s.hideGreek && !s.revealed[i];
        const gs = Object.assign({}, greekBase, {
          background: bg,
          cursor: s.hideGreek ? 'pointer' : 'default',
          transition: 'filter .18s, opacity .18s'
        });
        if (masked) Object.assign(gs, {
          filter: 'blur(5px)',
          userSelect: 'none',
          opacity: 0.5
        });
      ...
      if (ready) {
        const walk = cl => {
          if (cl.key) presentKeys[cl.key] = true;
          if (cl.kids) cl.kids.forEach(k => {
            if (k.kids) walk(k);
          });
        };
        u.periods.forEach(p => p.kids.forEach(k => {
      ...
      const legendActive = Object.keys(presentKeys).filter(k => this.clauseMeta[k]).map(k => ({
        label: this.clauseMeta[k].label,
        swatch: {
          width: '12px',
          height: '12px',
          borderRadius: '3px',
          background: this.clauseColor(k),
          flex: 'none'
        }
      }));
      const roleLegend = [];
      if (ready) {
        const seen = {};
        const L = s.dark ? 0.76 : 0.5;
        const scan = role => {
          if (!role) return;
          this.roleDefs.forEach(d => {
            if (d.re.test(role) && !seen[d.label]) {
              seen[d.label] = true;
              roleLegend.push({
                label: d.label,
                swatch: {
        ...
        const walk = cl => {
          cl.kids.forEach(c => {
            if (c.kids) walk(c);else scan(c.r);
          });
        };
        u.periods.forEach(p => p.kids.forEach(c => {
          if (c.kids) walk(c);else scan(c.r);
      ...
      return {
        ...
        ready,
        notReady: !ready && !s.err,
        err: s.err,
        ui: u ? {
          course: u.course,
          number: u.number,
          title: u.title,
          numLabel: ('0' + u.number).slice(-2)
        } : {
          course: 'Λατινικά Προσανατολισμού',
          number: '',
          title: 'Φόρτωση…',
          numLabel: '—'
        },
        parts,
        dirABtn: this.segStyle(s.dir === 'A'),
        dirBBtn: this.segStyle(s.dir === 'B'),
        onDirA: () => this.setDir('A'),
        onDirB: () => this.setDir('B'),
        showText: s.part === 'text' || s.printAll,
        showTrans: s.part === 'trans' || s.printAll,
        showNouns: s.part === 'nouns' || s.printAll,
        showCompar: s.part === 'compar' || s.printAll,
        ...
        showAnalysis: s.showAnalysis,
        hideGreek: s.hideGreek,
        syntaxBtn: this.pillStyle(s.showSyntax),
        analysisBtn: this.pillStyle(s.showAnalysis),
        greekBtn: this.pillStyle(s.hideGreek),
        greekBtnLabel: s.hideGreek ? '👁 Δείξε μετάφραση' : '⊘ Κρύψε μετάφραση',
        showArrows: s.showArrows,
        ...
        adminLabel: s.admin ? '🔓 Admin' : '🔒 Admin',
        onToggleAdmin: this.toggleAdmin,
        doExport: this.exportData,
        doReset: this.resetData,
        editL: e => this.editToken('l', e),
        editR: e => this.editToken('r', e),
        editTo: e => this.editToken('to', e),
        editG: e => this.editToken('g', e),
        editD: e => this.editToken('d', e),
        editNote: e => this.editToken('note', e),
        textView: this.buildText(),
        ...
        popStyle: {
          ...
          transform: s.pop.tf,
          zIndex: 9999,
          width: s.admin ? 'min(370px,92vw)' : 'min(330px,86vw)',
          maxHeight: '74vh',
          overflowY: 'auto',
          background: 'var(--panel)',
          border: '1px solid var(--line)',
          borderRadius: 'var(--r-lg)',
          boxShadow: '0 16px 44px -12px rgba(0,0,0,.34),0 3px 9px rgba(0,0,0,.12)',
          padding: '15px 16px',
          animation: 'ltpop .14s ease both'
        }
      };
    }
    render() {
      ...
      return /*#__PURE__*/React.createElement("div", {
        "data-root": true,
        ref: V.rootRef,
        style: s("height:100dvh;overflow:hidden;display:flex;flex-direction:column;background:var(--bg);color:var(--fg);font-family:var(--font-ui);--bg:#ecede7;--panel:#fbfcf8;--panel2:#eff1ea;--fg:#20261e;--muted:#5f6b5b;--line:#dae0d3;--line2:#e7ebe1;--accent:oklch(0.47 0.10 168);--accent2:oklch(0.42 0.11 168);--on-accent:#f4faf2;--hl:oklch(0.9 0.13 116);--font-ui:'Commissioner',system-ui,sans-serif;--font-serif:'EB Garamond',Georgia,serif;--font-latin:'EB Garamond',Georgia,serif;--font-mono:'IBM Plex Mono',monospace;--r:5px;--r-lg:10px;--body-dir:column;--nav-dir:row;--nav-w:100%;--nav-wrap:wrap;--nav-pad:11px clamp(16px,3vw,32px);--nav-bb:1px;--nav-br:0px;--maxw:1060px")
      }, /*#__PURE__*/React.createElement("header", {
        "data-chrome": true,
        style: s("display:flex;flex-wrap:wrap;gap:12px 20px;align-items:center;justify-content:space-between;padding:13px clamp(16px,3vw,32px);background:var(--panel);border-bottom:1px solid var(--line);position:sticky;top:0;z-index:60")
      }, /*#__PURE__*/React.createElement("div", {
        style: s("display:flex;align-items:center;gap:14px;min-width:0")
      }, /*#__PURE__*/React.createElement("div", {
        style: s("display:flex;align-items:center;justify-content:center;width:48px;height:48px;border-radius:var(--r);background:var(--accent);color:var(--on-accent);font-family:var(--font-serif);font-weight:800;font-size:21px;flex:none;letter-spacing:.01em")
      }, V.ui.numLabel), /*#__PURE__*/React.createElement("div", {
        style: s("min-width:0")
      }, /*#__PURE__*/React.createElement("div", {
        style: s("font-family:var(--font-ui);font-size:11px;letter-spacing:.14em;text-transform:uppercase;color:var(--muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis")
      }, /*#__PURE__*/React.createElement("span", {
        contentEditable: V.adminCE,
        suppressContentEditableWarning: true,
        "data-edit": "course"
      }, V.ui.course), " \xB7 \u0395\u03BD\u03CC\u03C4\u03B7\u03C4\u03B1 ", V.ui.number), /*#__PURE__*/React.createElement("div", {
        contentEditable: V.adminCE,
        suppressContentEditableWarning: true,
        "data-edit": "title",
        style: s("font-family:var(--font-serif);font-weight:800;font-size:clamp(16px,2vw,22px);line-height:1.12")
      }, V.ui.title))), /*#__PURE__*/React.createElement("div", {
        style: s("display:flex;align-items:center;gap:10px;flex-wrap:wrap")
      }, /*#__PURE__*/React.createElement("div", {
        style: s("display:flex;align-items:center;background:var(--panel2);border:1px solid var(--line);border-radius:999px;padding:5px 7px 5px 13px;gap:7px")
      }, /*#__PURE__*/React.createElement("span", {
        style: s("color:var(--muted);font-size:14px")
      }, "\u2315"), /*#__PURE__*/React.createElement("input", {
        value: V.query,
        onChange: V.onSearch,
        placeholder: "\u0391\u03BD\u03B1\u03B6\u03AE\u03C4\u03B7\u03C3\u03B7 \u03BB\u03AD\u03BE\u03B7\u03C2\u2026",
        style: s("border:0;background:transparent;outline:none;font-family:var(--font-ui);font-size:14px;color:var(--fg);width:150px")
      }), V.query && /*#__PURE__*/React.createElement("button", {
        onClick: V.clearSearch,
        style: s("border:0;background:var(--line);border-radius:999px;width:19px;height:19px;cursor:pointer;color:var(--fg);line-height:1;flex:none")
      }, "\xD7")), /*#__PURE__*/React.createElement("div", {
        style: s("display:flex;border:1px solid var(--line);border-radius:999px;overflow:hidden;background:var(--panel2)")
      }, /*#__PURE__*/React.createElement("button", {
        onClick: V.onDirA,
        style: V.dirABtn
      }, "\u0391 \xB7 \u039A\u03CE\u03B4\u03B9\u03BA\u03B1\u03C2"), /*#__PURE__*/React.createElement("button", {
        onClick: V.onDirB,
        style: V.dirBBtn
      }, "\u0392 \xB7 \u0395\u03C6\u03B1\u03C1\u03BC\u03BF\u03B3\u03AE")), /*#__PURE__*/React.createElement("button", {
        onClick: V.toggleDark,
        title: "\u0395\u03BD\u03B1\u03BB\u03BB\u03B1\u03B3\u03AE \u03C3\u03BA\u03BF\u03CD\u03C1\u03BF\u03C5 / \u03C6\u03C9\u03C4\u03B5\u03B9\u03BD\u03BF\u03CD \u03B8\u03AD\u03BC\u03B1\u03C4\u03BF\u03C2",
        style: V.darkBtn
      }, V.darkLabel), V.admin && /*#__PURE__*/React.createElement("button", {
        onClick: V.doExport,
        title: "\u0395\u03BE\u03B1\u03B3\u03C9\u03B3\u03AE \u03B4\u03B5\u03B4\u03BF\u03BC\u03AD\u03BD\u03C9\u03BD \u03B5\u03BD\u03CC\u03C4\u03B7\u03C4\u03B1\u03C2 (.js)",
        className: "lt-hova",
        style: s("display:inline-flex;align-items:center;gap:6px;cursor:pointer;border:1px solid var(--line);background:var(--panel);color:var(--fg);border-radius:999px;padding:7px 13px;font-family:var(--font-ui);font-size:13px;font-weight:600")
      }, "\u2B73 \u0395\u03BE\u03B1\u03B3\u03C9\u03B3\u03AE"), V.admin && /*#__PURE__*/React.createElement("button", {
        onClick: V.doReset,
        title: "\u0395\u03C0\u03B1\u03BD\u03B1\u03C6\u03BF\u03C1\u03AC \u03B1\u03C1\u03C7\u03B9\u03BA\u03CE\u03BD \u03B4\u03B5\u03B4\u03BF\u03BC\u03AD\u03BD\u03C9\u03BD",
        className: "lt-hova",
        style: s("cursor:pointer;border:1px solid var(--line);background:var(--panel);color:var(--muted);border-radius:999px;padding:7px 11px;font-family:var(--font-ui);font-size:13px;font-weight:600")
      }, "\u21BA"), V.admin && /*#__PURE__*/React.createElement("button", {
        onClick: V.doPrint,
        title: "\u0395\u03BA\u03C4\u03CD\u03C0\u03C9\u03C3\u03B7 / PDF",
        className: "lt-hova",
        ...
        style: V.adminBtn
      }, V.adminLabel))), /*#__PURE__*/React.createElement("div", {
        "data-body": true,
        style: s("display:flex;flex-direction:var(--body-dir);flex:1;min-height:0")
      }, /*#__PURE__*/React.createElement("nav", {
        "data-nav": true,
        style: s("display:flex;flex-direction:var(--nav-dir);gap:8px;width:var(--nav-w);padding:var(--nav-pad);background:var(--panel);border-right:var(--nav-br) solid var(--line);border-bottom:var(--nav-bb) solid var(--line);flex-wrap:var(--nav-wrap);align-content:flex-start")
      }, V.parts.map(p => /*#__PURE__*/React.createElement("button", {
        key: p.id,
        onClick: p.onSelect,
        style: p.style,
        className: "lt-hovb"
        ...
        style: p.rnStyle
      }, p.rn), /*#__PURE__*/React.createElement("span", null, p.label)))), /*#__PURE__*/React.createElement("main", {
        "data-scroll": true,
        ref: V.scrollRef,
        className: "lt-sc",
        style: s("position:relative;flex:1;min-height:0;overflow:auto;padding:clamp(18px,3vw,42px) clamp(16px,3vw,42px) 80px")
      }, /*#__PURE__*/React.createElement("div", {
        style: s("max-width:var(--maxw);margin:0 auto")
      }, V.notReady && /*#__PURE__*/React.createElement("div", {
        style: s("display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;padding:80px 20px;color:var(--muted);font-family:var(--font-ui)")
      }, /*#__PURE__*/React.createElement("div", {
        style: s("width:34px;height:34px;border:3px solid var(--line);border-top-color:var(--accent);border-radius:50%;animation:ltspin 1s linear infinite")
      }), /*#__PURE__*/React.createElement("div", null, "\u03A6\u03CC\u03C1\u03C4\u03C9\u03C3\u03B7 \u03B4\u03B5\u03B4\u03BF\u03BC\u03AD\u03BD\u03C9\u03BD \u03B5\u03BD\u03CC\u03C4\u03B7\u03C4\u03B1\u03C2\u2026")), V.err && /*#__PURE__*/React.createElement("div", {
        style: s("padding:40px 20px;color:var(--muted);font-family:var(--font-ui)")
      }, "\u26A0 \u03A3\u03C6\u03AC\u03BB\u03BC\u03B1 \u03C6\u03CC\u03C1\u03C4\u03C9\u03C3\u03B7\u03C2 \u03B4\u03B5\u03B4\u03BF\u03BC\u03AD\u03BD\u03C9\u03BD: ", V.err), V.ready && /*#__PURE__*/React.createElement(React.Fragment, null, V.showText && /*#__PURE__*/React.createElement("section", {
        "data-part": true,
        "data-screen-label": "\u039C\u03AD\u03C1\u03BF\u03C2 I \u2014 \u039A\u03B5\u03AF\u03BC\u03B5\u03BD\u03BF",
        style: s("animation:ltfade .4s ease both")
      }, /*#__PURE__*/React.createElement("div", {
        style: s("display:flex;flex-wrap:wrap;gap:12px;align-items:flex-end;justify-content:space-between;margin-bottom:8px")
      ...
      }, "\u039C\u03AD\u03C1\u03BF\u03C2 I"), /*#__PURE__*/React.createElement("h2", {
        ...
        style: s("font-family:var(--font-ui);font-size:14px;color:var(--muted);margin:0 0 18px;max-width:64ch;line-height:1.5")
      }, "\u03A0\u03AD\u03C1\u03B1\u03C3\u03B5 \u03C4\u03BF\u03BD \u03B4\u03B5\u03AF\u03BA\u03C4\u03B7 \u03C0\u03AC\u03BD\u03C9 \u03B1\u03C0\u03CC \u03BC\u03B9\u03B1 \u03BB\u03AD\u03BE\u03B7 (\u03AE \u03C0\u03AC\u03C4\u03B7\u03C3\u03AD \u03C4\u03B7\u03BD \u03C3\u03B5 \u03BA\u03B9\u03BD\u03B7\u03C4\u03CC/tablet) \u03B3\u03B9\u03B1 \u03C3\u03C5\u03BD\u03C4\u03B1\u03BA\u03C4\u03B9\u03BA\u03AE & \u03B3\u03C1\u03B1\u03BC\u03BC\u03B1\u03C4\u03B9\u03BA\u03AE \u03B1\u03BD\u03B1\u03B3\u03BD\u03CE\u03C1\u03B9\u03C3\u03B7. \u039C\u03B5 \u03C4\u03B7\u03BD ", /*#__PURE__*/React.createElement("b", {
        style: s("color:var(--accent)")
      }, "\u0391\u03BD\u03AC\u03BB\u03C5\u03C3\u03B7 \u03C3\u03CD\u03BD\u03C4\u03B1\u03BE\u03B7\u03C2"), " \u03B2\u03BB\u03AD\u03C0\u03B5\u03B9\u03C2 \u03CC\u03BB\u03BF \u03C4\u03BF \u03BA\u03B5\u03AF\u03BC\u03B5\u03BD\u03BF \u03C3\u03B5 ", /*#__PURE__*/React.createElement("b", null, "[ \u03BA\u03CD\u03C1\u03B9\u03B5\u03C2 ]"), " \u03BA\u03B1\u03B9 ", /*#__PURE__*/React.createElement("b", null, "( \u03B4\u03B5\u03C5\u03C4\u03B5\u03C1\u03B5\u03CD\u03BF\u03C5\u03C3\u03B5\u03C2 )"), " \u03C0\u03C1\u03BF\u03C4\u03AC\u03C3\u03B5\u03B9\u03C2, \u03BC\u03B5 \u03C7\u03C1\u03CE\u03BC\u03B1, \u03B5\u03C4\u03B9\u03BA\u03AD\u03C4\u03B1 \u03B5\u03AF\u03B4\u03BF\u03C5\u03C2 \u03BA\u03B1\u03B9 \u03C1\u03CC\u03BB\u03BF \u03BA\u03AC\u03B8\u03B5 \u03BB\u03AD\u03BE\u03B7\u03C2."), V.showSyntax && /*#__PURE__*/React.createElement("div", {
        style: s("display:flex;flex-wrap:wrap;gap:7px 14px;align-items:center;margin-bottom:16px;padding:11px 15px;background:var(--panel);border:1px solid var(--line);border-radius:var(--r-lg)")
      }, /*#__PURE__*/React.createElement("span", {
        style: s("font-family:var(--font-ui);font-size:12.5px;color:var(--muted);line-height:1.55")
      }, "\uD83C\uDFA8 \u039A\u03AC\u03B8\u03B5 \u03C0\u03C1\u03CC\u03C4\u03B1\u03C3\u03B7 \u03BA\u03B9 \u03BF\u03B9 \u03CC\u03C1\u03BF\u03B9 \u03C4\u03B7\u03C2 \u03BC\u03BF\u03B9\u03C1\u03AC\u03B6\u03BF\u03BD\u03C4\u03B1\u03B9 ", /*#__PURE__*/React.createElement("b", {
        style: s("color:var(--fg)")
      }, "\u03C4\u03BF \u03AF\u03B4\u03B9\u03BF \u03C7\u03C1\u03CE\u03BC\u03B1"), "\xB7 \u03BB\u03AD\u03BE\u03B7 \u03BC\u03B5 \u03B4\u03CD\u03BF \u03C1\u03AE\u03BC\u03B1\u03C4\u03B1 \u03C0\u03B1\u03AF\u03C1\u03BD\u03B5\u03B9 ", /*#__PURE__*/React.createElement("b", {
        style: s("color:var(--fg)")
      }, "\u03B4\u03CD\u03BF \u03C7\u03C1\u03CE\u03BC\u03B1\u03C4\u03B1"), ". \xA0\u20DD \u03C1\u03AE\u03BC\u03B1 \u03C3\u03B5 \u03BA\u03CD\u03BA\u03BB\u03BF \xB7 ", /*#__PURE__*/React.createElement("span", {
        style: s("text-decoration:underline;text-decoration-thickness:2px")
      }, "\u03B1\u03C0\u03B1\u03C1\u03AD\u03BC\u03C6\u03B1\u03C4\u03BF/\u03BC\u03B5\u03C4\u03BF\u03C7\u03AE"), " \u03C5\u03C0\u03BF\u03B3\u03C1\u03B1\u03BC\u03BC\u03B9\u03C3\u03BC\u03AD\u03BD\u03B1 \xB7 ", /*#__PURE__*/React.createElement("span", {
        style: s("text-decoration:underline wavy")
      }, "\u03B5\u03BC\u03C0\u03C1\u03CC\u03B8\u03B5\u03C4\u03BF\u03C2"), " \u03BC\u03B5 \u03BA\u03C5\u03BC\u03B1\u03C4\u03B9\u03C3\u03C4\u03AE. \u03A0\u03AD\u03C1\u03B1\u03C3\u03B5 \u03C0\u03AC\u03BD\u03C9 \u03B1\u03C0\u03CC \u03BB\u03AD\u03BE\u03B7 \u03B3\u03B9\u03B1 ", /*#__PURE__*/React.createElement("b", {
        style: s("color:var(--fg)")
      }, "\u03B2\u03B5\u03BB\u03AC\u03BA\u03B9"), " \u03C0\u03C1\u03BF\u03C2 \u03C4\u03BF\u03BD \u03CC\u03C1\u03BF \u03B5\u03BE\u03AC\u03C1\u03C4\u03B7\u03C3\u03B7\u03C2.")), /*#__PURE__*/React.createElement("div", {
        style: s("background:var(--panel);border:1px solid var(--line);border-radius:var(--r-lg);padding:clamp(20px,3.2vw,38px);box-shadow:0 1px 2px rgba(0,0,0,.03)")
      }, V.textView), V.showAnalysis && /*#__PURE__*/React.createElement("div", {
        style: s("margin-top:26px")
      }, /*#__PURE__*/React.createElement("h3", {
        style: s("font-family:var(--font-serif);font-weight:800;font-size:22px;margin:0 0 14px")
      }, "\u03A3\u03C5\u03BD\u03C4\u03B1\u03BA\u03C4\u03B9\u03BA\u03AE \u03B1\u03BD\u03AC\u03BB\u03C5\u03C3\u03B7"), /*#__PURE__*/React.createElement("div", {
        style: s("display:flex;flex-wrap:wrap;gap:7px 14px;align-items:center;margin-bottom:14px;padding:11px 15px;background:var(--panel);border:1px solid var(--line);border-radius:var(--r-lg)")
      }, /*#__PURE__*/React.createElement("span", {
        style: s("font-family:var(--font-ui);font-size:12.5px;color:var(--muted);line-height:1.55")
      }, "\uD83C\uDFA8 \u039A\u03AC\u03B8\u03B5 \u03C0\u03C1\u03CC\u03C4\u03B1\u03C3\u03B7 \u03BA\u03B9 \u03BF\u03B9 \u03C3\u03C5\u03BD\u03B4\u03B5\u03B4\u03B5\u03BC\u03AD\u03BD\u03BF\u03B9 \u03CC\u03C1\u03BF\u03B9 \u03C4\u03B7\u03C2 \u03AD\u03C7\u03BF\u03C5\u03BD ", /*#__PURE__*/React.createElement("b", {
        style: s("color:var(--fg)")
      }, "\u03C4\u03BF \u03AF\u03B4\u03B9\u03BF \u03C7\u03C1\u03CE\u03BC\u03B1"), "\xB7 \u03BB\u03AD\u03BE\u03B7 \u03C0\u03BF\u03C5 \u03B1\u03BD\u03AE\u03BA\u03B5\u03B9 \u03C3\u03B5 \u03B4\u03CD\u03BF \u03C1\u03AE\u03BC\u03B1\u03C4\u03B1 \u03B5\u03BC\u03C6\u03B1\u03BD\u03AF\u03B6\u03B5\u03C4\u03B1\u03B9 \u03BC\u03B5 ", /*#__PURE__*/React.createElement("b", {
        style: s("color:var(--fg)")
      }, "\u03B4\u03CD\u03BF \u03C7\u03C1\u03CE\u03BC\u03B1\u03C4\u03B1"), ".")), /*#__PURE__*/React.createElement("div", {
        style: s("display:flex;flex-direction:column;gap:11px")
      }, V.analysis.map(a => /*#__PURE__*/React.createElement("div", {
        key: a.key,
        style: a.card
      }, /*#__PURE__*/React.createElement("div", {
        style: s("display:flex;gap:10px;align-items:baseline;flex-wrap:wrap")
      }, /*#__PURE__*/React.createElement("span", {
        style: s("font-family:var(--font-mono);font-size:12px;color:var(--muted);flex:none")
      }, a.n, "."), /*#__PURE__*/React.createElement("span", {
        style: s("font-family:var(--font-latin);font-style:italic;font-size:17px")
      }, a.sentenceParts.map((sp, i) => /*#__PURE__*/React.createElement("span", {
        key: i,
        style: sp.style
      }, sp.t, " "))), /*#__PURE__*/React.createElement("span", {
        style: a.badge
      }, a.label)), a.note && /*#__PURE__*/React.createElement("div", {
        style: s("font-family:var(--font-ui);font-size:13px;color:var(--muted);margin-top:7px;line-height:1.5")
      }, a.note), /*#__PURE__*/React.createElement("div", {
        style: s("display:flex;flex-wrap:wrap;gap:5px 16px;margin-top:10px")
      }, a.items.map((it, i) => /*#__PURE__*/React.createElement("span", {
        key: i,
        style: s("display:inline-flex;align-items:center;gap:7px;font-family:var(--font-ui);font-size:13.5px;color:var(--fg)")
      }, /*#__PURE__*/React.createElement("span", {
        style: it.swatch
      }), /*#__PURE__*/React.createElement("b", {
        style: it.lStyle
      }, it.l), ": ", it.role)))))))), V.showTrans && /*#__PURE__*/React.createElement("section", {
        "data-part": true,
        "data-screen-label": "\u039C\u03AD\u03C1\u03BF\u03C2 II \u2014 \u039C\u03B5\u03C4\u03AC\u03C6\u03C1\u03B1\u03C3\u03B7",
        style: s("animation:ltfade .4s ease both")
      }, /*#__PURE__*/React.createElement("div", {
        style: s("display:flex;flex-wrap:wrap;gap:12px;align-items:flex-end;justify-content:space-between;margin-bottom:18px")
      }, /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
        style: s("font-family:var(--font-ui);font-size:11px;letter-spacing:.18em;text-transform:uppercase;color:var(--muted)")
      }, "\u039C\u03AD\u03C1\u03BF\u03C2 II"), /*#__PURE__*/React.createElement("h2", {
        style: s("margin:3px 0 0;font-family:var(--font-serif);font-weight:800;font-size:clamp(23px,3vw,32px)")
      }, "\u039C\u03B5\u03C4\u03AC\u03C6\u03C1\u03B1\u03C3\u03B7 \u03C3\u03B5 \u03B1\u03BD\u03C4\u03B9\u03C3\u03C4\u03BF\u03AF\u03C7\u03B9\u03C3\u03B7")), /*#__PURE__*/React.createElement("button", {
        onClick: V.toggleGreek,
        style: V.greekBtn
      }, V.greekBtnLabel)), V.hideGreek && /*#__PURE__*/React.createElement("p", {
        style: s("margin:-6px 0 16px;font-family:var(--font-ui);font-size:13px;color:var(--accent);font-weight:600")
      }, "\u2726 \u039B\u03B5\u03B9\u03C4\u03BF\u03C5\u03C1\u03B3\u03AF\u03B1 \u03B5\u03BE\u03AC\u03C3\u03BA\u03B7\u03C3\u03B7\u03C2 \u2014 \u03C0\u03AC\u03C4\u03B7\u03C3\u03B5 \u03BA\u03AC\u03B8\u03B5 \u03B3\u03C1\u03B1\u03BC\u03BC\u03AE \u03B3\u03B9\u03B1 \u03BD\u03B1 \u03B1\u03C0\u03BF\u03BA\u03B1\u03BB\u03CD\u03C8\u03B5\u03B9\u03C2 \u03C4\u03B7 \u03BC\u03B5\u03C4\u03AC\u03C6\u03C1\u03B1\u03C3\u03AE \u03C4\u03B7\u03C2."), /*#__PURE__*/React.createElement("div", {
        "data-align": true,
        style: s("display:grid;grid-template-columns:1fr 1fr;border:1px solid var(--line);border-radius:var(--r-lg);overflow:hidden")
      }, /*#__PURE__*/React.createElement("div", {
        style: s("padding:10px 16px;background:var(--panel2);font-family:var(--font-ui);font-size:11px;font-weight:800;letter-spacing:.1em;text-transform:uppercase;color:var(--muted)")
      }, "Latine"), /*#__PURE__*/React.createElement("div", {
        style: s("padding:10px 16px;background:var(--panel2);font-family:var(--font-ui);font-size:11px;font-weight:800;letter-spacing:.1em;text-transform:uppercase;color:var(--muted);border-left:1px solid var(--line2)")
      }, "\u0395\u03BB\u03BB\u03B7\u03BD\u03B9\u03BA\u03AC"), V.align.map((row, i) => /*#__PURE__*/React.createElement("div", {
        key: i,
        ...
        suppressContentEditableWarning: true,
        "data-edit": row.pLa
      }, row.la), /*#__PURE__*/React.createElement("div", {
        "data-el": true,
        style: row.elStyle,
        onClick: row.onReveal,
        contentEditable: V.adminCE,
        suppressContentEditableWarning: true,
        "data-edit": row.pEl
      }, row.el))))), V.showNouns && /*#__PURE__*/React.createElement("section", {
        "data-part": true,
        "data-screen-label": "\u039C\u03AD\u03C1\u03BF\u03C2 III \u2014 \u039F\u03C5\u03C3\u03B9\u03B1\u03C3\u03C4\u03B9\u03BA\u03AC & \u0395\u03C0\u03AF\u03B8\u03B5\u03C4\u03B1",
        style: s("animation:ltfade .4s ease both")
      }, /*#__PURE__*/React.createElement("div", {
        style: s("margin-bottom:8px")
      }, /*#__PURE__*/React.createElement("div", {
        style: s("font-family:var(--font-ui);font-size:11px;letter-spacing:.18em;text-transform:uppercase;color:var(--muted)")
      }, "\u039C\u03AD\u03C1\u03BF\u03C2 III"), /*#__PURE__*/React.createElement("h2", {
        style: s("margin:3px 0 0;font-family:var(--font-serif);font-weight:800;font-size:clamp(23px,3vw,32px)")
      }, "\u039F\u03C5\u03C3\u03B9\u03B1\u03C3\u03C4\u03B9\u03BA\u03AC & \u0395\u03C0\u03AF\u03B8\u03B5\u03C4\u03B1")), /*#__PURE__*/React.createElement("p", {
        style: s("font-family:var(--font-ui);font-size:13.5px;color:var(--muted);margin:0 0 20px")
      }, "\u03A4\u03B1\u03BE\u03B9\u03BD\u03BF\u03BC\u03B7\u03BC\u03AD\u03BD\u03B1 \u03BA\u03B1\u03C4\u03AC \u03BA\u03BB\u03AF\u03C3\u03B7 \u03BA\u03B1\u03B9 \u03B3\u03AD\u03BD\u03BF\u03C2. \u03A0\u03AC\u03C4\u03B7\u03C3\u03B5 \u03BC\u03B9\u03B1 \u03BB\u03AD\u03BE\u03B7 \u03B3\u03B9\u03B1 \u03BD\u03B1 \u03BC\u03B5\u03C4\u03B1\u03B2\u03B5\u03AF\u03C2 \u03C3\u03B5 \u03B1\u03C5\u03C4\u03AE\u03BD \u03BC\u03AD\u03C3\u03B1 \u03C3\u03C4\u03BF \u03BA\u03B5\u03AF\u03BC\u03B5\u03BD\u03BF."), /*#__PURE__*/React.createElement("h3", {
        style: s("font-family:var(--font-serif);font-weight:800;font-size:19px;margin:0 0 13px")
      }, "\u039F\u03C5\u03C3\u03B9\u03B1\u03C3\u03C4\u03B9\u03BA\u03AC"), /*#__PURE__*/React.createElement("div", {
        style: s("display:flex;flex-wrap:wrap;gap:14px;align-items:flex-start;margin-bottom:30px")
      }, V.nouns.map((kl, ki) => /*#__PURE__*/React.createElement("div", {
        key: ki,
        style: kl.cardStyle
      }, /*#__PURE__*/React.createElement("div", {
        style: s("font-family:var(--font-ui);font-size:12px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:var(--accent);margin:0 0 4px;padding-bottom:8px;border-bottom:2px solid var(--line)")
      }, kl.kl), kl.groups.map((g, gi) => /*#__PURE__*/React.createElement(React.Fragment, {
        key: gi
      }, /*#__PURE__*/React.createElement("div", {
        style: s("font-family:var(--font-ui);font-size:11px;font-weight:700;letter-spacing:.05em;text-transform:uppercase;color:var(--muted);margin:13px 0 8px")
      }, g.gender), /*#__PURE__*/React.createElement("div", {
        style: s("display:flex;flex-direction:column")
      }, g.items.map((it, ii) => /*#__PURE__*/React.createElement("div", {
        key: ii,
        onClick: it.onJump,
        className: "lt-hovc",
        style: s("display:flex;align-items:baseline;justify-content:space-between;flex-wrap:wrap;gap:1px 12px;width:100%;text-align:left;cursor:pointer;font-family:var(--font-ui);color:var(--fg);border-bottom:1px solid var(--line2);padding:7px 4px;transition:all .15s")
      }, /*#__PURE__*/React.createElement("span", {
        contentEditable: V.adminCE,
        suppressContentEditableWarning: true,
        "data-edit": it.pForm,
        style: s("font-family:var(--font-latin);font-style:italic;font-weight:600;font-size:16.5px")
      }, it.form), it.note && /*#__PURE__*/React.createElement("span", {
        contentEditable: V.adminCE,
        suppressContentEditableWarning: true,
        "data-edit": it.pNote,
        style: s("font-size:11.5px;color:var(--muted);white-space:nowrap;font-style:italic")
      }, it.note))))))))), /*#__PURE__*/React.createElement("h3", {
        style: s("font-family:var(--font-serif);font-weight:800;font-size:19px;margin:0 0 13px")
      }, "\u0395\u03C0\u03AF\u03B8\u03B5\u03C4\u03B1"), /*#__PURE__*/React.createElement("div", {
        style: s("display:flex;flex-wrap:wrap;gap:14px;align-items:flex-start")
      }, V.adjectives.map((kl, ki) => /*#__PURE__*/React.createElement("div", {
        key: ki,
        style: kl.cardStyle
      }, /*#__PURE__*/React.createElement("div", {
        style: s("font-family:var(--font-ui);font-size:12px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:var(--accent);margin:0 0 12px;padding-bottom:8px;border-bottom:2px solid var(--line)")
      }, kl.kl), /*#__PURE__*/React.createElement("div", {
        style: s("display:flex;flex-direction:column")
      }, kl.items.map((it, ii) => /*#__PURE__*/React.createElement("button", {
        key: ii,
        onClick: it.onJump,
        className: "lt-hova",
        style: s("display:flex;align-items:baseline;justify-content:space-between;flex-wrap:wrap;gap:1px 12px;width:100%;text-align:left;cursor:pointer;font-family:var(--font-ui);color:var(--fg);background:transparent;border:0;border-bottom:1px solid var(--line2);padding:7px 4px;transition:all .15s")
      }, /*#__PURE__*/React.createElement("span", {
        style: s("font-family:var(--font-latin);font-style:italic;font-weight:600;font-size:16.5px")
      }, it.form), it.note && /*#__PURE__*/React.createElement("span", {
        style: s("font-size:11.5px;color:var(--muted);white-space:nowrap;font-style:italic")
      }, it.note)))))))), V.showCompar && /*#__PURE__*/React.createElement("section", {
        "data-part": true,
        "data-screen-label": "\u039C\u03AD\u03C1\u03BF\u03C2 IV \u2014 \u03A0\u03B1\u03C1\u03B1\u03B8\u03B5\u03C4\u03B9\u03BA\u03AC",
        style: s("animation:ltfade .4s ease both")
      }, /*#__PURE__*/React.createElement("div", {
        style: s("margin-bottom:8px")
      }, /*#__PURE__*/React.createElement("div", {
        style: s("font-family:var(--font-ui);font-size:11px;letter-spacing:.18em;text-transform:uppercase;color:var(--muted)")
      }, "\u039C\u03AD\u03C1\u03BF\u03C2 IV"), /*#__PURE__*/React.createElement("h2", {
        style: s("margin:3px 0 0;font-family:var(--font-serif);font-weight:800;font-size:clamp(23px,3vw,32px)")
      }, "\u03A0\u03B1\u03C1\u03B1\u03B8\u03B5\u03C4\u03B9\u03BA\u03AC \u0395\u03C0\u03B9\u03B8\u03AD\u03C4\u03C9\u03BD")), /*#__PURE__*/React.createElement("p", {
        style: s("font-family:var(--font-ui);font-size:13.5px;color:var(--muted);margin:0 0 20px")
      }, "\u0398\u03B5\u03C4\u03B9\u03BA\u03CC\u03C2 \xB7 \u03A3\u03C5\u03B3\u03BA\u03C1\u03B9\u03C4\u03B9\u03BA\u03CC\u03C2 \xB7 \u03A5\u03C0\u03B5\u03C1\u03B8\u03B5\u03C4\u03B9\u03BA\u03CC\u03C2 \u03B2\u03B1\u03B8\u03BC\u03CC\u03C2, \u03BA\u03B1\u03C4\u03AC \u03BA\u03BB\u03AF\u03C3\u03B7."), V.comparatives.map((kl, ki) => /*#__PURE__*/React.createElement("div", {
        key: ki,
        style: s("margin-bottom:22px")
      }, /*#__PURE__*/React.createElement("div", {
        style: s("font-family:var(--font-ui);font-size:12px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:var(--accent);margin:0 0 10px")
      }, kl.kl), /*#__PURE__*/React.createElement("div", {
        ...
        style: s("text-align:left;font-family:var(--font-ui);font-size:11px;font-weight:800;letter-spacing:.05em;text-transform:uppercase;color:var(--muted);padding:11px 16px;background:var(--panel2)")
      }, "\u03A3\u03C5\u03B3\u03BA\u03C1\u03B9\u03C4\u03B9\u03BA\u03CC\u03C2"), /*#__PURE__*/React.createElement("th", {
        style: s("text-align:left;font-family:var(--font-ui);font-size:11px;font-weight:800;letter-spacing:.05em;text-transform:uppercase;color:var(--muted);padding:11px 16px;background:var(--panel2)")
      }, "\u03A5\u03C0\u03B5\u03C1\u03B8\u03B5\u03C4\u03B9\u03BA\u03CC\u03C2"))), /*#__PURE__*/React.createElement("tbody", null, kl.rows.map((r, ri) => /*#__PURE__*/React.createElement("tr", {
        key: ri,
        className: "lt-hovr"
        ...
        style: s("margin-bottom:18px")
      }, /*#__PURE__*/React.createElement("div", {
        style: s("font-family:var(--font-ui);font-size:11px;letter-spacing:.18em;text-transform:uppercase;color:var(--muted)")
      }, "\u039C\u03AD\u03C1\u03BF\u03C2 V"), /*#__PURE__*/React.createElement("h2", {
        style: s("margin:3px 0 0;font-family:var(--font-serif);font-weight:800;font-size:clamp(23px,3vw,32px)")
        ...
        style: s("text-align:left;font-family:var(--font-ui);font-size:11px;font-weight:800;letter-spacing:.05em;text-transform:uppercase;color:var(--muted);padding:11px 16px;background:var(--panel2)")
      }, "\u0395\u03AF\u03B4\u03BF\u03C2"), /*#__PURE__*/React.createElement("th", {
        style: s("text-align:left;font-family:var(--font-ui);font-size:11px;font-weight:800;letter-spacing:.05em;text-transform:uppercase;color:var(--muted);padding:11px 16px;background:var(--panel2)")
      }, "\u03A0\u03B1\u03C1\u03B1\u03C4\u03AE\u03C1\u03B7\u03C3\u03B7"))), /*#__PURE__*/React.createElement("tbody", null, V.pronouns.map((p, pi) => /*#__PURE__*/React.createElement("tr", {
        key: pi,
        className: "lt-hovr"
      }, /*#__PURE__*/React.createElement("td", {
        ...
        style: s("font-family:var(--font-ui);font-size:11px;letter-spacing:.18em;text-transform:uppercase;color:var(--muted)")
      }, "\u039C\u03AD\u03C1\u03BF\u03C2 VI"), /*#__PURE__*/React.createElement("h2", {
        style: s("margin:3px 0 0;font-family:var(--font-serif);font-weight:800;font-size:clamp(23px,3vw,32px)")
      }, "\u0391\u03C1\u03C7\u03B9\u03BA\u03BF\u03AF \u03A7\u03C1\u03CC\u03BD\u03BF\u03B9 \u03A1\u03B7\u03BC\u03AC\u03C4\u03C9\u03BD")), /*#__PURE__*/React.createElement("p", {
        style: s("font-family:var(--font-ui);font-size:13.5px;color:var(--muted);margin:0 0 20px")
      }, "\u03A4\u03B1\u03BE\u03B9\u03BD\u03BF\u03BC\u03B7\u03BC\u03AD\u03BD\u03B1 \u03BA\u03B1\u03C4\u03AC \u03C3\u03C5\u03B6\u03C5\u03B3\u03AF\u03B1. \u03A0\u03AC\u03C4\u03B7\u03C3\u03B5 \u03C4\u03BF\u03BD \u03B5\u03BD\u03B5\u03C3\u03C4\u03CE\u03C4\u03B1 \u03B3\u03B9\u03B1 \u03BC\u03B5\u03C4\u03AC\u03B2\u03B1\u03C3\u03B7 \u03C3\u03C4\u03BF \u03BA\u03B5\u03AF\u03BC\u03B5\u03BD\u03BF."), V.verbs.map((sz, zi) => /*#__PURE__*/React.createElement("div", {
        key: zi,
        style: s("margin-bottom:22px")
      }, /*#__PURE__*/React.createElement("div", {
        style: s("font-family:var(--font-ui);font-size:12px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:var(--accent);margin:0 0 10px")
        ...
        style: s("text-align:left;font-family:var(--font-ui);font-size:11px;font-weight:800;letter-spacing:.04em;text-transform:uppercase;color:var(--muted);padding:10px 14px;background:var(--panel2)")
      }, "\u03A0\u03B1\u03C1\u03B1\u03BA."), /*#__PURE__*/React.createElement("th", {
        style: s("text-align:left;font-family:var(--font-ui);font-size:11px;font-weight:800;letter-spacing:.04em;text-transform:uppercase;color:var(--muted);padding:10px 14px;background:var(--panel2)")
      }, "\u03A3\u03BF\u03C5\u03C0."), /*#__PURE__*/React.createElement("th", {
        style: s("text-align:left;font-family:var(--font-ui);font-size:11px;font-weight:800;letter-spacing:.04em;text-transform:uppercase;color:var(--muted);padding:10px 14px;background:var(--panel2)")
      }, "\u0391\u03C0\u03B1\u03C1\u03AD\u03BC\u03C6."), /*#__PURE__*/React.createElement("th", {
        style: s("text-align:left;font-family:var(--font-ui);font-size:11px;font-weight:800;letter-spacing:.04em;text-transform:uppercase;color:var(--muted);padding:10px 14px;background:var(--panel2)")
      }, "\u03A0\u03B1\u03C1\u03B1\u03C4."))), /*#__PURE__*/React.createElement("tbody", null, sz.rows.map((r, ri) => /*#__PURE__*/React.createElement("tr", {
        key: ri,
        className: "lt-hovr"
      }, /*#__PURE__*/React.createElement("td", {
        ...
        style: s("font-family:var(--font-ui);font-size:13px;color:var(--muted);padding:12px 16px;border:1px dashed var(--line);border-radius:var(--r-lg)")
      }, "\u2014 \u03B4\u03B5\u03BD \u03B1\u03C0\u03B1\u03BD\u03C4\u03AC \u03C1\u03AE\u03BC\u03B1 \u03B1\u03C5\u03C4\u03AE\u03C2 \u03C4\u03B7\u03C2 \u03C3\u03C5\u03B6\u03C5\u03B3\u03AF\u03B1\u03C2 \u03C3\u03C4\u03B7\u03BD \u03B5\u03BD\u03CC\u03C4\u03B7\u03C4\u03B1")))), V.showSos && /*#__PURE__*/React.createElement("section", {
        "data-part": true,
        "data-screen-label": "\u039C\u03AD\u03C1\u03BF\u03C2 VII \u2014 SOS",
        style: s("animation:ltfade .4s ease both")
      }, /*#__PURE__*/React.createElement("div", {
        style: s("margin-bottom:8px")
      }, /*#__PURE__*/React.createElement("div", {
        style: s("font-family:var(--font-ui);font-size:11px;letter-spacing:.18em;text-transform:uppercase;color:var(--muted)")
      }, "\u039C\u03AD\u03C1\u03BF\u03C2 VII \xB7 \u03A0\u03C1\u03BF\u03B1\u03B9\u03C1\u03B5\u03C4\u03B9\u03BA\u03CC"), /*#__PURE__*/React.createElement("h2", {
        style: s("margin:3px 0 0;font-family:var(--font-serif);font-weight:800;font-size:clamp(23px,3vw,32px)")
        ...
        style: s("font-family:var(--font-latin);font-style:italic;font-weight:800;font-size:20px;color:var(--fg)")
      }, V.pop.l), /*#__PURE__*/React.createElement("button", {
        onClick: V.closePop,
        style: s("border:0;background:var(--panel2);border-radius:999px;width:23px;height:23px;cursor:pointer;color:var(--muted);flex:none;line-height:1")
      }, "\xD7")), V.notAdmin && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
        style: s("display:inline-block;margin-top:8px;font-family:var(--font-ui);font-size:11px;font-weight:800;letter-spacing:.05em;text-transform:uppercase;color:var(--on-accent);background:var(--accent);padding:3px 9px;border-radius:5px")
      }, V.pop.r), /*#__PURE__*/React.createElement("div", {
        style: s("margin-top:11px;font-family:var(--font-ui);font-size:13.5px;color:var(--fg);line-height:1.5")
      }, V.pop.g), /*#__PURE__*/React.createElement("div", {
        style: s("margin-top:7px;font-family:var(--font-latin);font-size:15px;font-style:italic;color:var(--muted)")
      }, V.pop.d), V.pop.note && /*#__PURE__*/React.createElement("div", {
        style: s("margin-top:10px;padding-top:10px;border-top:1px dashed var(--line);font-family:var(--font-ui);font-size:12.5px;color:var(--muted);line-height:1.5")
      }, "\u279F ", V.pop.note)), V.admin && /*#__PURE__*/React.createElement("div", {
        style: s("display:flex;flex-direction:column;gap:9px;margin-top:11px")
      }, /*#__PURE__*/React.createElement("label", {
        style: s("font-family:var(--font-ui);font-size:10px;font-weight:800;letter-spacing:.06em;text-transform:uppercase;color:var(--muted)")
      }, "\u039B\u03B1\u03C4\u03B9\u03BD\u03B9\u03BA\u03CC\u03C2 \u03C4\u03CD\u03C0\u03BF\u03C2", /*#__PURE__*/React.createElement("input", {
        value: V.pop.l,
        onChange: V.editL,
        style: s("display:block;width:100%;margin-top:3px;font-family:var(--font-latin);font-style:italic;font-size:14px;padding:6px 9px;border:1px solid var(--line);border-radius:7px;background:var(--panel2);color:var(--fg)")
      })), /*#__PURE__*/React.createElement("label", {
        style: s("font-family:var(--font-ui);font-size:10px;font-weight:800;letter-spacing:.06em;text-transform:uppercase;color:var(--muted)")
      }, "\u03A3\u03C5\u03BD\u03C4\u03B1\u03BA\u03C4\u03B9\u03BA\u03CC\u03C2 \u03C1\u03CC\u03BB\u03BF\u03C2", /*#__PURE__*/React.createElement("input", {
        value: V.pop.rawR,
        onChange: V.editR,
        style: s("display:block;width:100%;margin-top:3px;font-family:var(--font-ui);font-size:13px;padding:6px 9px;border:1px solid var(--line);border-radius:7px;background:var(--panel2);color:var(--fg)")
      })), /*#__PURE__*/React.createElement("label", {
        style: s("font-family:var(--font-ui);font-size:10px;font-weight:800;letter-spacing:.06em;text-transform:uppercase;color:var(--muted)")
      }, "\u0395\u03BE\u03AC\u03C1\u03C4\u03B7\u03C3\u03B7 (\u03C0.\u03C7. \xAB\u03C3\u03C4\u03BF gerunt\xBB)", /*#__PURE__*/React.createElement("input", {
        value: V.pop.toRaw,
        onChange: V.editTo,
        style: s("display:block;width:100%;margin-top:3px;font-family:var(--font-ui);font-size:13px;padding:6px 9px;border:1px solid var(--line);border-radius:7px;background:var(--panel2);color:var(--fg)")
      })), /*#__PURE__*/React.createElement("label", {
        style: s("font-family:var(--font-ui);font-size:10px;font-weight:800;letter-spacing:.06em;text-transform:uppercase;color:var(--muted)")
      }, "\u0393\u03C1\u03B1\u03BC\u03BC\u03B1\u03C4\u03B9\u03BA\u03AE", /*#__PURE__*/React.createElement("textarea", {
        value: V.pop.g,
        onChange: V.editG,
        rows: "2",
        ...
        style: s("font-family:var(--font-ui);font-size:10px;font-weight:800;letter-spacing:.06em;text-transform:uppercase;color:var(--muted)")
      }, "\u039B\u03AE\u03BC\u03BC\u03B1 \u2013 \u03C3\u03B7\u03BC\u03B1\u03C3\u03AF\u03B1", /*#__PURE__*/React.createElement("textarea", {
        value: V.pop.d,
        onChange: V.editD,
        rows: "2",
        style: s("display:block;width:100%;margin-top:3px;font-family:var(--font-ui);font-size:13px;padding:6px 9px;border:1px solid var(--line);border-radius:7px;background:var(--panel2);color:var(--fg);resize:vertical")
        ...
        style: s("font-family:var(--font-ui);font-size:10px;font-weight:800;letter-spacing:.06em;text-transform:uppercase;color:var(--muted)")
      }, "\u03A3\u03B7\u03BC\u03B5\u03AF\u03C9\u03C3\u03B7 (\u03C0\u03C1\u03BF\u03B1\u03B9\u03C1\u03B5\u03C4\u03B9\u03BA\u03CC)", /*#__PURE__*/React.createElement("textarea", {
        value: V.pop.note,
        onChange: V.editNote,
        rows: "2",
        style: s("display:block;width:100%;margin-top:3px;font-family:var(--font-ui);font-size:13px;padding:6px 9px;border:1px solid var(--line);border-radius:7px;background:var(--panel2);color:var(--fg);resize:vertical")
