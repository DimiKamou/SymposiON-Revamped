...
| crypto-hack | `CryptoHack.open({...})` (IIFE; **no static overlay** — created at runtime) | game.js | *(dynamic; `ch-overlay` 1743 is the card-host)* | game.css | SC (questions via cfg) |
...
(`ARV`, `EIMI`, `KR`, `LIV`, `Halieia`, `GoldenFleece`, `CryptoHack`, `LiveArena`, `Mnemosyne`) and
...
| Crypto Hack | `CryptoHack.open({...})` | IIFE, dynamic overlay |
...
Note: `crypto-hack` and `live-arena` create/use their overlays specially (crypto-hack builds its own at
runtime → `overlay:null`; live-arena uses `la-overlay` and is eager).
...
- **COLLISION: eager-only games can't be lazy.** `iliada-arcade` inits on `DOMContentLoaded`;
  ...
  **Decision:** load these eagerly in the revamp (`eager:true` in manifest), or refactor live-arena to
  `window.LiveArena` + guarded handlers first. `crypto-hack` (CryptoHack IIFE, builds own overlay) is
  launchable lazily via `CryptoHack.open()` but has no static overlay — handle `overlay:null`.
